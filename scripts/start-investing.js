...
projectsDb.sort((a, b) => {
  return (
    b.raisedFull.substring(1).replace(",", "") -
    a.raisedFull.substring(1).replace(",", "")
  );
});
...
function displayProjects(projectsDb) {
  projectsDb.forEach((project) => {
    ...
    tombStoneContentDiv.append(
      tombStoneContentHeading,
      tombStoneContentTagline
    );
    ...
    tombstoneDiv.append(
      tombStoneCoverDiv,
      tombStoneCoverBannerDiv,
      tombStoneContentDiv
    );
    document.querySelector(".all-tombstones").appendChild(tombstoneDiv);

    if (industriesObj.hasOwnProperty(project.category))
      industriesObj[project.category]++;
...
function orderByFunding() {
  ...
  if (selectedFunding == "mostFunded") {
    ...
    filteredObjects.sort((a, b) => {
      return (
        a.investorsFull.replace(",", "") - b.investorsFull.replace(",", "")
      );
    });
    removeProjects();
    displayProjects(filteredObjects);
...
function filterProgress() {
  ...
  if (selectedProgress == "default" || selectedProgress == "inProgress") {
    ...
    let filteredObjects = [];
    projectsDb.forEach((project) => {
      if (project.progressStatus == selectedProgress)
        filteredObjects.push(project);
    });
    removeProjects();
    displayProjects(filteredObjects);
    document.querySelector(".text-success").textContent =
...
document
  .querySelector(".investment-type")
  .addEventListener("change", filterProgress);
